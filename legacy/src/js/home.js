...
function fetch_user_details(token) {
    return fetch('http://localhost:3000/api/logins/home.json', {
            method: 'POST',
            headers: {
...
function navbar(option) {
    ...
    if (option === 'hide' && window.innerWidth <= 1024 ) { // small screens
        nav.style.display = "none";
    }
    
    if (option === 'show' && window.innerWidth <= 1024) { // small screens
...
function join_submit() {
    ...
    fetch(`http://localhost:3000/api/work/${work_admin_value}/${url_value}/join.json`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
